...
用法:
    python seed_ehr_data.py --patient-id <patient_id> --json-file <path_to_json>
    python seed_ehr_data.py --patient-id <patient_id> --json-file <path_to_json> --document-id <doc_id>
    python seed_ehr_data.py --patient-id <patient_id> --json-file <path_to_json> --clear
    python seed_ehr_data.py --patient-id <patient_id> --clear-only
...
功能:
    ...
    5. 自动同时写入 patient_ehr 和 project_crf 两种实例
    6. 兼容前端 ehrData.ts（病历夹详情页）和 ProjectDatasetView（科研项目抽取详情页）

设计:
    - 不依赖 ehr_pipeline.py，可独立运行
    - 通用：任意符合 patient_ehr_v2 schema 结构的 JSON 都能入库
    - 幂等：重复运行会更新已有字段的 selected 值
    - 双写：同时写入 patient_ehr 和 project_crf 实例
"""
...
PROJECT_CRF_SCHEMA_CODE = "patient_ehr_v2_crf"
...
def get_or_create_instance(conn: sqlite3.Connection, patient_id: str, schema_id: str,
                           instance_type: str = "patient_ehr", name: str = "电子病历夹") -> str:
    """确保 schema_instances 中存在该患者的实例，返回 instance_id。"""
    cur = conn.cursor()
    cur.execute(
        """
    ...
    cur.execute(
        ...
        VALUES (?, ?, ?, ?, ?, 'draft', ?, ?)
        """,
        (new_id, patient_id, schema_id, instance_type, name, _now_iso(), _now_iso()),
    )
    return new_id


def get_schema_id(conn: sqlite3.Connection, schema_code: str) -> Optional[str]:
    """获取 schema 的 UUID id。"""
    cur = conn.cursor()
    cur.execute(
...
def seed_fields(
    ...
    返回 (total, new, updated, sourced, row_candidate_count) 计数。
    """
    now = _now_iso()
...
def seed_single_instance(
    ...
    """对单个实例执行 clear + seed 流程。"""
    print(f"\n{'─'*50}")
    print(f"📋 [{instance_label}] Instance: {instance_id}")
    ...
    if do_clear:
        ...
        print(f"   🧹 已清空: selected={n_sel}, candidates={n_cand}")

    run_id = create_extraction_run(conn, instance_id, document_id)
    print(f"   🔬 Extraction run: {run_id}")

    total, new_count, updated_count, sourced_count, row_cand_count = seed_fields(
        conn, instance_id, fields, document_id, run_id, sources_map
    )

    print(f"   ✅ 总字段: {total} | 新增: {new_count} | 更新: {updated_count} | 有溯源: {sourced_count} | 逐行候选: {row_cand_count}")


# ─── 主流程 ────────────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="通用 EHR JSON 入库工具（双写 patient_ehr + project_crf）")
    ...
    parser.add_argument(
        "--clear",
        action="store_true",
        help="入库前清空该患者实例下的 field_value_selected / field_value_candidates / extraction_runs",
    )
    parser.add_argument(
        "--clear-only",
        action="store_true",
        help="仅执行上述清空，不读取 JSON、不入库",
    )
    parser.add_argument(
        "--skip-project-crf",
        action="store_true",
        help="仅写入 patient_ehr 实例，不自动写入 project_crf 实例",
    )
    ...
    try:
        # ─── 获取 schema IDs ────────────────────────────────────────────────────
        patient_ehr_schema_id = get_schema_id(conn, args.schema_code)
        if not patient_ehr_schema_id:
            print(f"❌ schemas 表中不存在 code={args.schema_code}", file=sys.stderr)
            sys.exit(1)

        project_crf_schema_id = get_schema_id(conn, PROJECT_CRF_SCHEMA_CODE)
        # project_crf schema 可能不存在，不是致命错误

        # ─── clear-only 模式 ─────────────────────────────────────────────────────
        if args.clear_only:
            instance_id = get_or_create_instance(conn, args.patient_id, patient_ehr_schema_id, "patient_ehr")
            clear_instance_ehr_rows(conn, instance_id)
            print(f"🧹 已清空 patient_ehr instance={instance_id}")

            if project_crf_schema_id and not args.skip_project_crf:
                crf_instance_id = get_or_create_instance(conn, args.patient_id, project_crf_schema_id, "project_crf")
                clear_instance_ehr_rows(conn, crf_instance_id)
                print(f"🧹 已清空 project_crf instance={crf_instance_id}")
        ...
        # ─── 写入 patient_ehr 实例 ───────────────────────────────────────────────
        print(f"\n🗂️  Schema (patient_ehr): {args.schema_code}, id={patient_ehr_schema_id[:20]}...")
        patient_ehr_instance_id = get_or_create_instance(
            conn, args.patient_id, patient_ehr_schema_id, "patient_ehr", "电子病历夹"
        )
        seed_single_instance(
            conn, patient_ehr_instance_id, "patient_ehr",
            fields, args.document_id, sources_map, args.clear,
        )

        # ─── 写入 project_crf 实例 ──────────────────────────────────────────────
        if project_crf_schema_id and not args.skip_project_crf:
            print(f"\n🗂️  Schema (project_crf): {PROJECT_CRF_SCHEMA_CODE}, id={project_crf_schema_id[:20]}...")
            project_crf_instance_id = get_or_create_instance(
                conn, args.patient_id, project_crf_schema_id, "project_crf", "科研CRF数据"
            )
            seed_single_instance(
                conn, project_crf_instance_id, "project_crf",
                fields, args.document_id, sources_map, args.clear,
            )
        else:
            if args.skip_project_crf:
                print(f"\n⏭️  跳过 project_crf（--skip-project-crf）")
            else:
                print(f"\n⚠️  未找到 {PROJECT_CRF_SCHEMA_CODE} schema，跳过 project_crf 写入")
        ...
        print(f"{'='*50}")
        print(f"\n💡 现在刷新前端页面即可看到填充效果（病历夹详情页 + 科研项目抽取详情页）")
