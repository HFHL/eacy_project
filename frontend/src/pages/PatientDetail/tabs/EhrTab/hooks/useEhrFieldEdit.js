...
import { updatePatientEhr } from '@/api/patient'
...
export const useEhrFieldEdit = (patientId = null, onSaveSuccess = null) => {
  ...
  const handleEhrSaveEdit = useCallback(async (fieldId) => {
    ...
    setSaving(true)
    try {
      const res = await updatePatientEhr(patientId, {
        fields: [{ field_id: fieldId, value: editingEhrValue }]
      })
  ...
  const handleEhrBatchEdit = useCallback(async (fieldUpdates) => {
    ...
    try {
      const res = await updatePatientEhr(patientId, {
        fields: fieldUpdates.map(f => ({ field_id: f.fieldId, value: f.value }))
      })
