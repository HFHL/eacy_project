    ...
    return (
      ...
      >
        ...
        <iframe
          title={fileName || '原始文档'}
          src={imageUrl}
          style={{ width: '100%', minHeight: 560, border: 0, borderRadius: 4, background: '#fff' }}
        />
        <div style={{
          marginTop: 12,
          padding: '8px 12px',
    ...
    if (Array.isArray(value)) {
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {value.map((item, idx) => (
            <div key={idx} style={{ background: '#f5f5f5', padding: '6px 10px', borderRadius: 4 }}>
              ...
              {typeof item === 'object' && item !== null ? (
                <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 8 }}>
                  {Object.entries(item).filter(([_, v]) => v != null && v !== '').slice(0, 5).map(([k, v]) => (
                    <span key={k}>
                      ...
                      <Text style={{ color: '#1890ff' }}>{String(v)}</Text>
                    </span>
                  ))}
                ...
                <Text strong style={{ color: '#1890ff' }}>{String(item)}</Text>
              )}
            </div>
          ))}
        </div>
            ...
            {docInfo ? (
              <Row gutter={16}>
                <Col span={8}>
                  ...
                  <div>
                    <Text code style={{ fontSize: 11 }}>{docId?.slice(0, 8)}...</Text>
                  </div>
                </Col>
                <Col span={8}>
