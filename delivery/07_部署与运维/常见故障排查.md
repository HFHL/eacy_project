...
| 根因 | 排查命令 / 检查点 |
|---|---|
| OCR worker 未启动 | `docker compose ... ps worker-ocr` / `inspect ping` |
...
| Redis 不可达 | `docker compose ... exec redis redis-cli ping` |
...
1. `docker compose -f docker-compose.prod.yml --env-file .env.prod exec redis redis-cli -n 11 llen ocr` 确认堆积
2. `docker compose -f docker-compose.prod.yml --env-file .env.prod restart worker-ocr`，观察日志
3. 若是 TextIn 错：核对 [[环境变量清单#六TextIn OCR]] 中四个变量，重启 OCR worker

### 相关日志

- 路径：`docker compose -f docker-compose.prod.yml --env-file .env.prod logs -f worker-ocr`
- 关键字：`TEXTIN`、`process_document_ocr`、`ConnectionRefusedError`

### 预防措施

- [[监控与告警]] 中开启 `ocr` 队列长度告警
- compose 中的 `restart: unless-stopped` 保证 worker 崩溃自动拉起
...
3. **重启 `worker-metadata` + `worker-extraction`**（参考 [[Celery任务运维]]）
4. 在前端或后端 API 重试失败任务
...
- Nginx 配置 `client_max_body_size 200m` 见 `deploy/docker/nginx/eacy.conf`
...
- API 与所有 worker 共用同一份 `.env.prod` / `EACY_ENV_FILE`
- 谨慎给读路径加缓存，必须有失效机制
...
| Celery 并发过高 | 看 `OCR_CONCURRENCY` / `METADATA_CONCURRENCY` / `EXTRACTION_CONCURRENCY` 总和 |
...
2. 降 `EXTRACTION_CONCURRENCY` 到 1，重启 `worker-extraction` 后观察恢复
...
1. `docker compose -f docker-compose.prod.yml --env-file .env.prod run --rm api alembic current` 记录当前位置
2. 读最近一个迁移脚本，定位失败语句
3. 如是数据冲突：先手动 `UPDATE` 让旧数据合法，再 `docker compose -f docker-compose.prod.yml --env-file .env.prod run --rm migrate`
