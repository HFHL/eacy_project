...
sequenceDiagram
    ...
    alt 任务仍在运行
        FE->>EV: useExtractionProgressSSE 轮询 events
        loop Worker 推进
            TPS->>DB: UPDATE async_task_items<br/>(progress/stage/heartbeat)
            TPS->>DB: INSERT async_task_events
            FE->>API: GET /admin/extraction-tasks/{batch_id}/events?after_id=...
            API-->>FE: 新增事件（时间线）
        end
        EV-->>FE: 检测到 terminal 事件 (done/error)
